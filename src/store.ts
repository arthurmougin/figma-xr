...
export const useStore = defineStore("store", {
	...
	actions: {
		...
		login() {
			...
			const figmaOAuthUrl = new URL(`https://www.figma.com/oauth`);
			figmaOAuthUrl.searchParams.set(
				"client_id",
		...
		async initiateAuth() {
			...
			if (
				urlParams.get("state") !=
				(await localForage.getItem("figma_state"))
			) {
				throw new Error(
			...
			try {
				...
				const base64EncodedClientIDAndSecret = btoa(
					`${import.meta.env.VITE_FIGMA_CLIENT_ID}:${
						import.meta.env.VITE_FIGMA_SECRET
					}`
				);
				headers.set(
					"Authorization",
				...
				const body = `redirect_uri=${encodeURIComponent(
					redirect_uri
				...
				const expireTime =
					parseInt(json.expires_in) * 1000 + Date.now();
